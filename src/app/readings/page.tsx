...
// The spoken dialogue framing a reading — what the reader says, and the reply.
...
const REPLY: React.CSSProperties = { color: "var(--gold-deep)", fontStyle: "normal", fontWeight: 600 };
...
export default function ReadingsPage() {
  ...
  // "<title>. <greeting> <intro> <refrain> <body> <acclamation>", so each block
  // starts where the blocks above it end. Keep these in step with `segments`.
  const speaking = narration.status !== "idle";
  const titleWords = countWords(reading.title);
  const greetingWords = reading.greeting ? countWords(reading.greeting.say) : 0;
  ...
  const greetingOffset = titleWords;
  const introOffset = greetingOffset + greetingWords;
    ...
    <div className="pw-readings" style={{ display: "flex", gap: 0, alignItems: "flex-start", minHeight: "100%", overflowX: "hidden" }}>
      ...
      <div className="pw-readings-main" style={{ flex: 1, padding: "40px 48px 72px", minWidth: 0, maxWidth: "100%", overflowX: "hidden" }}>
        ...
        <div style={{ maxWidth: 700 }}>
          ...
          {/* The dialogue that opens the reading: the Gospel is greeted first
              ("The Lord be with you.") and then announced; the other readings are
              only announced. The psalm has neither — it is the people's response
              to the first reading, not a reading itself. */}
          {(reading.greeting || reading.intro) && (
            <div style={{ ...DIALOGUE, margin: "0 0 24px" }}>
              {reading.greeting && (
                <div>
                  <SpokenText as="span" text={reading.greeting.say} active={speaking} wordIndex={narration.wordIndex} wordOffset={greetingOffset} />
                  <span style={REPLY}>{" "}— {reading.greeting.reply}</span>
                </div>
              )}
              {reading.intro && (
                <div style={reading.greeting ? { marginTop: 6 } : undefined}>
                  <SpokenText as="span" text={reading.intro} active={speaking} wordIndex={narration.wordIndex} wordOffset={introOffset} />
                  {reading.introReply && <span style={REPLY}>{" "}— {reading.introReply}</span>}
                </div>
              )}
            </div>
          )}
          ...
          {reading.acclamation && (
            <div style={DIALOGUE}>
              ...
              <span style={REPLY}>{" "}— {reading.acclamation.reply}</span>
            </div>
          )}
